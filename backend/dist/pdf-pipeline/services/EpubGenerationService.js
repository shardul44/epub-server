...
export class EpubGenerationService {
     ...
     */
    static async generate(layout, coords, options = {}, elements = [], htmlPageFiles = []) {
        ...
        const pdf2htmlCssFiles = await EpubGenerationService.copyPdf2htmlCss(layout.css, path.join(epubDir, 'css'));
        await EpubGenerationService.writeDefaultCss(path.join(epubDir, 'css', 'fxl.css'), fontFiles);
        // Generate XHTML pages
        const xhtmlFiles = await XhtmlGenerationService.generatePages(epubDir, coords, imageFiles, elements, htmlPageFiles);
        // Generate SMIL overlays
        const smilFiles = await SmilGenerationService.generate(path.join(epubDir, 'smil'), coords);
        // Also copy SMIL to job smil dir
        for (const smilFile of smilFiles) {
            await fs.copyFile(path.join(epubDir, 'smil', smilFile), path.join(layout.smil, smilFile)).catch(() => { });
        }
    ...
    static async copyDir(srcDir, destDir) {
        const copied = [];
        try {
    ...
    static async writeDefaultCss(cssPath, fontFiles) {
...
`;
    ...
    static buildFontFaceRules(fontFiles) {
        return fontFiles
            .map((filePath) => EpubGenerationService.buildFontFaceRule(filePath))
            .filter(Boolean)
            .join('\n');
    }
    static buildFontFaceRule(filePath) {
    ...
    static defaultFontFamily(fontFiles) {
    ...
    static imageMediaType(filePath) {
    ...
    static async writeContainer(epubDir) {
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    ...
    static async writeOpf(epubDir, meta) {
        ...
        for (let i = 0; i < meta.xhtmlFiles.length; i++) {
            ...
            const id = `page_${i + 1}`;
            manifestItems.push(`<item id="${id}" href="${escapeXml(xhtml)}" media-type="application/xhtml+xml"/>`);
            if (meta.smilFiles[i]) {
                const smilId = `smil_${i + 1}`;
                manifestItems.push(`<item id="${smilId}" href="smil/${escapeXml(meta.smilFiles[i])}" media-type="application/smil+xml"/>`);
        ...
        for (let i = 0; i < meta.imageFiles.length; i++) {
            ...
            manifestItems.push(`<item id="img_${i + 1}" href="${escapeXml(href)}" media-type="${mediaType}"/>`);
        }
        for (let i = 0; i < meta.fontFiles.length; i++) {
            ...
            manifestItems.push(`<item id="font_${i + 1}" href="${escapeXml(href)}" media-type="${mediaType}"/>`);
        }
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="book-id">
  ...
  <spine>
    ...
    static async writeNav(epubDir, pages, title) {
        const navItems = pages
            .map((p) => `        <li><a href="pages/page_${p.number}.xhtml">Page ${p.number}</a></li>`)
            .join('\n');
...
${navItems}
    ...
    static async zipEpub(epubDir, outputPath) {
        ...
        return new Promise((resolve, reject) => {
            ...
            archive.pipe(output);
            archive.append(mimetypeBuf, { name: 'mimetype', store: true });
...
//# sourceMappingURL=EpubGenerationService.js.map
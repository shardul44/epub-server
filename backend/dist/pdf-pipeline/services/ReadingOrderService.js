...
export class ReadingOrderService {
    static reconstruct(elements, options = {}) {
     ...
     */
    static detectColumns(elements, gapThreshold) {
     ...
     */
    static sortWithinColumn(elements, lineTolerance) {
        ...
        const lines = [];
        for (const el of sorted) {
            const line = lines.find((ln) => ln.length > 0 && Math.abs(ln[0].y - el.y) <= lineTolerance);
            if (line) {
                line.push(el);
            }
            else {
...
//# sourceMappingURL=ReadingOrderService.js.map
...
export class PdfToHtmlService {
  static readonly DOCKER_IMAGE = process.env.PDF2HTMLEX_IMAGE || 'guoxuequan/pdf2htmlex';
  ...
  static async convert(
    ...
    await withRetry(
      () => PdfToHtmlService.runDocker(dockerArgs),
      { label: 'pdf2htmlEX', maxAttempts: 2, baseDelayMs: 2000 }
    );
  ...
  private static runDocker(args: string[]): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        ...
        reject(
          new Error(
            `pdf2htmlEX exited with code ${code}\nstdout: ${stdout.slice(-2000)}\nstderr: ${stderr.slice(-2000)}`
          )
        );
      });
    });
  ...
  private static async organizeOutput(
    workDir: string,
    layout: JobDirectoryLayout
  ): Promise<PdfToHtmlResult> {
    ...
    if (htmlFiles.length === 0) {
      ...
      if (mainStat) htmlFiles.push(mainHtml);
    }

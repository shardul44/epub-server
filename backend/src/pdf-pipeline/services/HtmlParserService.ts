...
interface CssPositionMap {
  x: Map<string, number>;
  y: Map<string, number>;
  w: Map<string, number>;
  h: Map<string, number>;
  fs: Map<string, number>;
  // matrix transforms: maps class name -> [a, b, c, d] from matrix(a,b,c,d,e,f)
  m: Map<string, [number, number, number, number]>;
}
 ...
 */
function parseMatrix(css: string): [number, number, number, number] | null {
 ...
 */
function isRotatedMatrix(mat: [number, number, number, number]): boolean {
...
export class HtmlParserService {
   ...
   */
  static async parseHtmlFiles(
    htmlFiles: string[],
    cssFiles: string[]
  ): Promise<{ elements: HtmlTextElement[]; pages: Array<{ number: number; width: number; height: number }> }> {
    const cssMap = await HtmlParserService.parseCssFiles(cssFiles);
    const allElements: HtmlTextElement[] = [];
    const pages: Array<{ number: number; width: number; height: number }> = [];
  ...
  private static inferPageNumber(htmlPath: string, html: string, fallback: number): number {
    ...
    if (dataMatch) return parseInt(dataMatch[1], 16);

    const idMatch = /id=["']pf([0-9a-f]+)["']/i.exec(html);
    if (idMatch) return parseInt(idMatch[1], 16);
  ...
  private static parseSingleHtml(
    html: string,
    pageNumber: number,
    cssMap: CssPositionMap
  ): { elements: HtmlTextElement[]; width: number; height: number } {
    const $ = cheerio.load(html);
    const elements: HtmlTextElement[] = [];
    ...
    if (pageFrame.length) {
      ...
      if (wClass && cssMap.w.has(wClass)) pageWidth = cssMap.w.get(wClass)! * defaultScale;
      if (hClass && cssMap.h.has(hClass)) pageHeight = cssMap.h.get(hClass)! * defaultScale;
    }
    ...
    $('.t').each((_, el) => {
      ...
      if (mat && isRotatedMatrix(mat)) return;
  ...
  private static resolveFontFamily(classes: string[]): string | undefined {
    const ffClass = classes.find((cls) => /^ff[0-9a-f]+$/i.test(cls));
    return ffClass ? ffClass.toLowerCase() : undefined;
  }

  private static extractClass(node: { attr: (name: string) => string | undefined }, prefix: string): string | null {
  ...
  private static resolvePosition(
    ...
    for (const cls of classes) {
      ...
      if (cssMap.w.has(cls)) width = cssMap.w.get(cls)!;
      if (cssMap.h.has(cls)) height = cssMap.h.get(cls)!;
      if (cssMap.fs.has(cls)) fontSize = cssMap.fs.get(cls)!;
    }

    if (!hasPosition) return null;
    if (width <= 0) width = Math.max(20, (fontSize || 12) * 3);

    return { x, y, width, height, fontSize };
  }

  private static parseStylePx(style: string, prop: string): number | null {
  ...
  private static async parseCssFiles(cssFiles: string[]): Promise<CssPositionMap> {
    const map: CssPositionMap = {
      x: new Map(),
      y: new Map(),
      w: new Map(),
      h: new Map(),
  ...
  private static parseCssContent(css: string, map: CssPositionMap): void {
    ...
    while ((match = ruleRe.exec(css)) !== null) {
      ...
      if (top !== null && className.startsWith('y')) map.y.set(className, top);
      if (width !== null && className.startsWith('w')) map.w.set(className, width);
      if (height !== null && className.startsWith('h')) map.h.set(className, height);
      if (fontSize !== null && className.startsWith('fs')) map.fs.set(className, fontSize);
  ...
  private static parseCssValue(body: string, prop: string): number | null {
    const re = new RegExp(`${prop}\\s*:\\s*([\\d.]+)(px|pt)?`, 'i');
    const m = re.exec(body);
    if (!m) return null;
    const val = parseFloat(m[1]);
    const unit = m[2]?.toLowerCase();
    if (unit === 'pt') return val * PT_TO_PX;
    return val;
  }
}

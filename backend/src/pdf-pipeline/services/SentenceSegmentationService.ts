...
export class SentenceSegmentationService {
   ...
   */
  static segmentByElements(
    elements: HtmlTextElement[],
    words: WordModel[]
  ): SentenceModel[] {
    const wordsByElement = new Map<number, WordModel[]>();
    for (const word of words) {
      const idx = word.elementIndex ?? 0;
      if (!wordsByElement.has(idx)) wordsByElement.set(idx, []);
      wordsByElement.get(idx)!.push(word);
    }

    const rawSentences: Array<{ text: string; words: string[]; page: number; domIndex?: number }> =
      [];
  ...
  static segment(words: WordModel[]): SentenceModel[] {
    ...
    const rawSentences: Array<{ text: string; words: string[]; page: number }> = [];
    let currentWords: WordModel[] = [];

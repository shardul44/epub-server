...
export class CoordinateService {
  ...
  static validate(coords: CoordsJson): void {
    if (!Array.isArray(coords.pages)) throw new Error('coords.pages must be an array');
    if (!Array.isArray(coords.words)) throw new Error('coords.words must be an array');
    if (!Array.isArray(coords.sentences)) throw new Error('coords.sentences must be an array');
  ...
  static async load(coordsPath: string): Promise<CoordsJson> {
    const raw = await fs.readFile(coordsPath, 'utf8');
    const coords = JSON.parse(raw) as CoordsJson;
    CoordinateService.validate(coords);
    return coords;
  }
}
